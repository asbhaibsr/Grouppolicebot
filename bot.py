...
from config import (
    ...
    COMMAND_COOLDOWN_TIME, MESSAGE_REPLY_COOLDOWN_TIME,
...
from database import (
    ...
    add_owner_taught_response, get_owner_taught_response, add_conversational_learning,
    get_conversational_learning_response, get_group_status_flags, update_group_status_flag
)
from filters import (
...
chat_message_cooldowns = {} # Cooldowns for general replies per chat
...
def check_cooldown(user_id, cooldown_type="command"):
    ...
    elif cooldown_type == "message_reply":
        # Chat-specific cooldown
...
async def button_callback_handler(client: Client, query):
    ...
    # Log button interaction (आपके स्निपेट के आधार पर)
    if database.buttons_collection:
        database.buttons_collection.insert_one({
            "query_id": query.id,
    ...
    if action == "toggle":
        ...
        help_text = (
            ...
            "• `/teach <trigger> = <response>`: (केवल मालिक) बॉट को विशिष्ट ट्रिगर्स पर प्रतिक्रिया देना सिखाएं।\n\n"
...
async def generate_settings_keyboard(group_id):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(f"यूज़रनेम फ़िल्टर: {'ON' if group_settings.get('usernamedel_enabled') else 'OFF'}", callback_data=f"toggle_usernamedel_enabled_{group_id}")], # आपके स्निपेट से
...
async def handle_group_message(client: Client, message: Message):
    ...
    # 4. यूज़रनेम फ़िल्टर (आपके स्निपेट से)
    ...
    # --- लर्निंग बॉट का जवाब (यदि कोई उल्लंघन नहीं है और बॉट सक्षम है) ---
    if not violation_detected and check_cooldown(chat.id, "message_reply"):
        # पहले मालिक द्वारा सिखाए गए जवाब देखें
        response_data = get_owner_taught_response(original_content.lower())
        if response_data:
            await message.reply_text(response_data["response"], parse_mode=ParseMode.MARKDOWN)
            return

        # फिर सामान्य सीखने के जवाब देखें
        response_data = get_conversational_learning_response(original_content.lower())
        if response_data:
            await message.reply_text(response_data["response"], parse_mode=ParseMode.MARKDOWN)
            return
...
async def handle_new_chat_members(client: Client, message: Message):
    ...
    if message.new_chat_members and client.me.id in [member.id for member in message.new_chat_members]:
        ...
        add_or_update_group(message.chat.id, message.chat.title, message.from_user.id if message.from_user else None)
...
@app.on_message(filters.command("teach") & filters.user(OWNER_ID) & filters.private)
async def teach_command(client: Client, message: Message):
    ...
    if len(command_args) < 2:
        await message.reply_text("उपयोग: `/teach <trigger> = <response>`")
        return

    parts = command_args[1].split('=', 1)
    if len(parts) < 2:
        await message.reply_text("अमान्य प्रारूप। उपयोग: `/teach <trigger> = <response>`")
        return

    trigger = parts[0].strip().lower()
    response = parts[1].strip()

    add_owner_taught_response(trigger, response)
    await message.reply_text(f"बॉट ने सीखा: '{trigger}' का जवाब '{response}' है।")

# सामान्य बातचीत सीखने के लिए (यदि मालिक द्वारा सेट किया गया है)
@app.on_message(filters.command("learn") & filters.user(OWNER_ID) & filters.group)
async def learn_command(client: Client, message: Message):
    if not check_cooldown(message.from_user.id, "command"):
        return

    if not message.reply_to_message or not message.reply_to_message.text:
        await message.reply_text("कृपया उस संदेश का जवाब दें जिसे आप बॉट को सीखने के लिए ट्रिगर के रूप में उपयोग करना चाहते हैं, और फिर अपना प्रतिक्रिया संदेश लिखें।\n"
                                 "उदाहरण: `/learn` (उत्तर में) <आपका प्रतिक्रिया संदेश>")
        return

    trigger = message.reply_to_message.text.strip().lower()
    response = message.text.split(None, 1)[1].strip() if len(message.text.split()) > 1 else None

    if not response:
        await message.reply_text("कृपया प्रतिक्रिया संदेश प्रदान करें।")
        return

    add_conversational_learning(trigger, response)
    await message.reply_text(f"बॉट ने बातचीत से सीखा: '{trigger}' का जवाब '{response}' है।")
