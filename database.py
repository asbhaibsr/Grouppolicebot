...
violations_collection = db["violations"] if db else None
biolink_exceptions_collection = db["biolink_exceptions"] if db else None
new_entries_log_collection = db["new_entries_log"] if db else None
# Chatbot-related collections removed.

# --- Group Operations ---
def add_or_update_group(group_id: int, group_name: str, added_by_user_id: int = None):
    if not groups_collection: return
